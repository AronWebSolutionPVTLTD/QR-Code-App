...
function useField(input, dependencies = []) {
  ...
  const runValidation2 = (0, import_react2.useCallback)(
    (value2 = state.value) => {
...
function asChoiceList({
  ...
  return {
    ...fieldData,
    selected: value === void 0 || value === null ? [] : [value],
    onChange: (selected) => {
      onChange(selected[0]);
    },
    allowMultiple: false
...
function useBaseList(listOrConfig, validationDependencies = []) {
  ...
  const validationConfigs = (0, import_react5.useMemo)(
    () => mapObject(validates, normalizeValidation),
...
function useLazyRef(getValue) {
  const [value] = (0, import_react10.useState)(getValue);
...
function createUseMediaFactory(useEffectHook) {
  return (query) => {
    const [match, setMatch] = (0, import_react11.useState)(false);
    useEffectHook(() => {
...
var useMedia = createUseMediaFactory(import_react11.useEffect);
var useMediaLayout = createUseMediaFactory(import_react11.useLayoutEffect);
...
function useSubmit(onSubmit = noopSubmission, fieldBag, makeCleanAfterSubmit = false, dynamicListBag) {
  ...
  const [submitErrors, setSubmitErrors] = (0, import_react19.useState)([]);
  const fieldBagRef = useLazyRef(() => fieldBag);
  fieldBagRef.current = fieldBag;
  const dynamicListBagRef = useLazyRef(() => dynamicListBag);
  const setErrors = (0, import_react19.useCallback)((errors) => {
    setSubmitErrors(errors);
    propagateErrors(fieldBagRef.current, errors);
  }, [fieldBagRef]);
  const submit = (0, import_react19.useCallback)(async (event) => {
...
function useForm({
  ...
  const fieldsWithLists = (0, import_react21.useMemo)(() => {
    if (dynamicLists) {
  ...
  const makeClean = (0, import_react21.useCallback)(() => {
